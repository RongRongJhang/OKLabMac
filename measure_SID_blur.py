...
def metrics(im_dir, label_dir, use_GT_mean, loss_fn):
    avg_psnr = 0
    avg_ssim = 0
    avg_lpips = 0
    n = 0
...
if __name__ == '__main__':
    ...
    if mea.Blur:
        for index in range(1, 257):
            ...
            if os.path.exists(test_dir + fill_index):
                i_psnr, i_ssim, i_lpips, i_n = metrics(im_dir, label_dir, mea.use_GT_mean, loss_fn)
                print(f"===> Finish {fill_index} folder")
                print(f"===> Avg.PSNR: {i_psnr/i_n:.4f} dB")
                print(f"===> Avg.SSIM: {i_ssim/i_n:.4f}")
                print(f"===> Avg.LPIPS: {i_lpips.item()/i_n:.4f}\n")
                avg_psnr += i_psnr
                avg_ssim += i_ssim
    ...
    print("===> All Finish")
    print(f"===> Avg.PSNR: {avg_psnr/n:.4f} dB")
    print(f"===> Avg.SSIM: {avg_ssim/n:.4f}")
    print(f"===> Avg.LPIPS: {avg_lpips/n:.4f}")
